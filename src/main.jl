...
function exponents_new_generating_set(genset)
    ...
    for (varidx, var) in enumerate(gens(basepolyring))
        ...
        for point in points
            ...
            # @info "" Ideal
            gb = std(Ideal, complete_reduction=true)
...
function naive_new_generating_set(genset)
    ...
    gb = collect(gens(std(Ideal, complete_reduction=true)))
    println(gb)
        
    gb = [
        deux_singular2aa(f, base=basepolyring, new_ring=parent(t))
        for f in gb
    ]
...
function display_generating_exponents(genset)
    ...
    for v in keys(inter) 
        ...
        for (idx, g) in enumerate(exps)
            println("\ngenerator $idx, degrees of coefficients")
            for ee in keys(g)
...
function new_generating_set(genset)
    ...
    lightring, = Singular.PolynomialRing(GROUND, [ystrings..., "t"])
...
R, (x1, x2, x3) = AbstractAlgebra.PolynomialRing(GROUND, ["x1", "x2", "x3"])

Q = FractionField(R)

set = [ (x1 + x2 + x3 + 8) // (x2 + 1), (x2 + 1) // (x1 + 1234)]
# set = load_generators("../data/Goodwin.txt")
# set = map(s -> change_base_ring(GROUND, numerator(s)) // change_base_ring(GROUND, denominator(s)), set)


naive = naive_new_generating_set(set)

newset = new_generating_set(set)

println("set = ", set)
println("interpolated ", newset)
println("true ", naive)
